 ...
 <form method="post" class="form-horizontal">
    ...
    <div class="form-group">
      ...
      <select name="country_name" class="form-control">
		  ...
		  mysqli_select_db("myphpdb",$con);
		  $res=mysqli_query("select * from tblcountry");
    ...
    <div class="form-group">
      ...
      <div class="col-sm-10">
        <select name="state_name" class="form-control" id="classdd">
		  ...
		  $con=mysqli_connect("localhost","root","");
		  mysqli_select_db("myphpdb",$con);
		  $res=mysqli_query("select * from tblstate");
		  while($row=mysqli_fetch_array($res))
		  {
		  ?>
              <option value="<?php echo $row["name"]; ?>"><?php echo $row["name"]; ?></option>
    ...
    <div class="form-group">
      ...
      <div class="col-sm-10">
        <select name="district_name" class="form-control" id="classdd">
		  ...
		  $con=mysqli_connect("localhost","root","");
		  mysqli_select_db("myphpdb",$con);
		  $res=mysqli_query("select * from tbldistrict");
		  while($row=mysqli_fetch_array($res))
		  {
		  ?>
              <option value="<?php echo $row["name"]; ?>"><?php echo $row["name"]; ?></option>
...
if(isset($_POST['submit']))
{
   ...
   $qry="INSERT INTO `myphpdb`.`tbladmlkg` (`id`, `fstname`, `lstname`, `father's_name`, `mother's_name`, `email_ID`, `class`, `dob`, `gender`, `mobile_no`, `addr`, `city`, `country`, `state`, `district`, `pincode`) VALUES ('NULL', '$fstname', '$lstname', '$father_name', '$mother_name', '$email', '$class_name', '$dob', '$gender', '$mobileno', '$addresss', '$city', '$country', '$state', '$district', '$pincode')";
  $run=mysqli_query($qry,$con);
   if($run==true)
   {
       ...
       <?php
echo "<script>window.location='printadmissionform.php'</script>";
   }
